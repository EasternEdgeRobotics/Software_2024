...
export function MainCamera(props: {ip: string, width: number, enableHud: boolean}) {
    return (
        <Box height={(props.width) * (1473/5152)} sx={{backgroundImage: `url(${props.ip}), url(./nosignal.jpg)`, backgroundSize: "100% 100%", borderRadius: "12px", margin: "0 4px 0 4px"}}>
...
export function AltCamera(props: {ip: string, width: number}) {
    return (
        <Box height={props.width * (1473/5152)} sx={{backgroundImage: `url(${props.ip}), url(./nosignal.jpg)`, backgroundSize: "100% 100%", borderRadius: "12px"}}/>
    );
}
