...
export default function ProfileEditor(props: {open: boolean; onClose: () => void}) {
	...
	const getProfileInfo = (profileName: string) => {
		for (let i = 0; i<profilesList.length;i++){
			if (profilesList[i].name == profileName){
				return profileName + ", Controller 1: " + profilesList[i].controller1 + ", Controller 2: " + profilesList[i].controller2
			} 
		}
		return profileName + ", Controller 1: " + "Unassigned" + ", Controller 2: " + "Unassigned"
	}
