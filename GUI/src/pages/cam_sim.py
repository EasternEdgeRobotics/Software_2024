...
class SimulationCameraSubscriber(Node):
	...
	def __init__(self):
		...
		self.simulation_camera_listener = self.create_subscription(
			Image, '/demo_cam/camera0/image_raw', self.simulation_camera_0_listener_callback, 10)
		self.simulation_camera_listener = self.create_subscription(
			Image, '/demo_cam/camera1/image_raw', self.simulation_camera_1_listener_callback, 10)
		self.simulation_camera_listener = self.create_subscription(
			Image, '/demo_cam/camera2/image_raw', self.simulation_camera_2_listener_callback, 10)
		self.simulation_camera_listener = self.create_subscription(
			Image, '/demo_cam/camera3/image_raw', self.simulation_camera_3_listener_callback, 10)
...
class CamHandler(BaseHTTPRequestHandler):
	def do_GET(self):
		...
		# Will either return 8880, 8881, 8882, or 8883
		self.port_number = self.server.server_address[1]
		# Will either return 0, 1, 2, or 3
		self.camera_number = self.server.server_address[1] - 8880
		...
		if self.path.endswith('.mjpg'):
			...
			while True:
				try:
					imgRGB = cv2.cvtColor(
						camera_captures[self.camera_number], cv2.COLOR_BGR2RGB)
		...
		if self.path.endswith('.html'):
			...
			self.wfile.write(
				f'<img src="http://{ip_address}:{self.port_number}/cam.mjpg"/>')
...
def main(args=None):
	...
	camera_0_server = threading.Thread(
		target=runserver, args=(8880,), daemon=True)
	camera_1_server = threading.Thread(
		target=runserver, args=(8881,), daemon=True)
	camera_2_server = threading.Thread(
		target=runserver, args=(8882,), daemon=True)
	camera_3_server = threading.Thread(
		target=runserver, args=(8883,), daemon=True)
