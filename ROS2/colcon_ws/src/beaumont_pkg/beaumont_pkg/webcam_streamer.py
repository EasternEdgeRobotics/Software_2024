...
def snapshot_frame():
    success, frame = camera.read()
    if not success:
        return "Could not read frame from camera. Make sure the camera is connected and not being used by another process."
    else:
        ret, buffer = cv2.imencode('.jpg', frame)
        frame = buffer.tobytes()
        yield (b'--frame\r\n'
...
def snapshot():
    return Response(snapshot_frame(), mimetype='multipart/x-mixed-replace; boundary=frame')
